...
\begin{proof}
 ...
 $\P_{H_{0}}(H|x) = \phi(H|x)$ when $\phi(H|x) \in \{0,1\}$, and  $\P_{H_{0}}(H_0|x) = 1$, if $\phi(H_0|x) = \half$. Let $\sP = \{\P_{H_0}: H_0 \in \sigma(\Theta)\}$, and $\preceq$ and $\ll$ be such as in \cref{def:pref-1,def:pref-2}. Next, we show that $\ll$ satisfies four conditions that are analogous to \cref{def:rep}.
 ...
 The proof follows from \cref{lemma:nonstd_1,lemma:nonstd_2} and \citet{Schervish2022}.
\end{proof}
...
\begin{lemma}
 \label{lemma:region-is-coherent}
 If $\phi$ is a region-based test,
 then it is completely-coherent.
\end{lemma}
